...
<div class="max-w-4xl space-y-10">
    ...
    <form action="{{ isset($banner) ? route('admin.hero.update', $banner) : route('admin.hero.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-[3.5rem] shadow-sm border border-zinc-100 p-10 md:p-16 space-y-10">
        @csrf 
        ...
        <div class="grid md:grid-cols-2 gap-10">
            <div class="space-y-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="tag" value="{{ $banner->tag ?? '' }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900" placeholder="#IndonesiaBerbagi">
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-2">Judul Slide (Mendukung HTML)</label>
                    <input type="text" name="title" value="{{ $banner->title ?? '' }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900">
                </div>
            </div>
            <div class="space-y-2">
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" rows="3" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-medium text-zinc-600">{{ $banner->description ?? '' }}</textarea>
        </div>

        <div class="grid md:grid-cols-3 gap-6">
            <div class="space-y-2">
                <label class="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-2">Teks Tombol</label>
                <input type="text" name="cta_text" value="{{ $banner->cta_text ?? 'Mulai Berdonasi' }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900">
            </div>
            <div class="space-y-2">
                <label class="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-2">Link Tujuan</label>
                <input type="text" name="cta_link" value="{{ $banner->cta_link ?? '/donasi' }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900">
            </div>
            <div class="space-y-2">
                <label class="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-2">Urutan</label>
                <input type="number" name="order" value="{{ $banner->order ?? 0 }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900">
            </div>
        </div>
