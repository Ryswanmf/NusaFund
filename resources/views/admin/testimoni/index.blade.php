...
<div class="space-y-10">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            ...
            <p class="text-zinc-500 mt-2 font-medium">Kisah kebaikan dari donatur dan penerima manfaat.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-50">
                <thead class="bg-zinc-50/50">
                    <tr>
                        <th class="px-10 py-6 text-left text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">Profil</th>
                ...
                <tbody class="divide-y divide-zinc-50">
                    ...
                    <tr class="hover:bg-zinc-50/50 transition">
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-zinc-100 overflow-hidden flex-shrink-0">
                                    @if($t->avatar)
                                        <img src="{{ asset('storage/' . $t->avatar) }}" class="w-full h-full object-cover">
                                    @else
                                        <div class="w-full h-full flex items-center justify-center text-zinc-400 font-bold">NF</div>
                                    @endif
                                </div>
                                <div class="min-w-0">
                                    <p class="text-sm font-black text-zinc-900 truncate">{{ $t->name }}</p>
                                    <p class="text-[10px] font-bold text-maroon-600 uppercase tracking-widest">{{ $t->role }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                            <p class="text-sm text-zinc-500 font-medium italic line-clamp-2 max-w-md">"{{ $t->message }}"</p>
                        </td>
                        <td class="px-10 py-6 text-right space-x-2">
                            <a href="{{ route('admin.testimoni.edit', $t) }}" class="inline-flex p-2.5 bg-zinc-50 text-zinc-400 hover:text-maroon-700 rounded-xl transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <form action="{{ route('admin.testimoni.destroy', $t) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus testimoni ini?')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="3" class="px-10 py-20 text-center text-zinc-400">Belum ada testimoni.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
