...
<div class="max-w-4xl space-y-10">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-zinc-900 tracking-tight">Edit <span class="text-maroon-700">Kategori</span></h1>
            <p class="text-zinc-500 mt-2 font-medium">Perbarui informasi dan ikon kategori donasi.</p>
        </div>
    ...
    <form action="{{ route('admin.kategori.update', $kategori) }}" method="POST" class="bg-white rounded-[3.5rem] shadow-sm border border-zinc-100 p-8 md:p-16 space-y-10" x-data="{ iconCode: '{{ addslashes($kategori->icon_svg) }}' }">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <div class="space-y-2">
                ...
                <input type="text" name="name" value="{{ old('name', $kategori->name) }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900">
            ...
            <div class="grid md:grid-cols-2 gap-10">
                <div class="space-y-2">
                    ...
                    <textarea name="icon_svg" rows="5" required class="w-full bg-zinc-50 border-none rounded-[2rem] py-6 px-8 focus:ring-2 focus:ring-maroon-500 font-mono text-xs text-zinc-600 leading-relaxed" x-model="iconCode"></textarea>
                    <p class="text-[10px] text-zinc-400 mt-2 pl-2">Gunakan path SVG (hanya bagian d="..." atau seluruh tag &lt;svg&gt;).</p>
            ...
            <div class="flex items-center gap-4 p-6 bg-zinc-50 rounded-3xl border border-zinc-100">
                <input type="checkbox" name="is_featured" id="is_featured" {{ $kategori->is_featured ? 'checked' : '' }} class="w-6 h-6 rounded-lg border-zinc-200 text-maroon-700 focus:ring-maroon-500">
