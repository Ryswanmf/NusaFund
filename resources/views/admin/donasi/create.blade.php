...
<div class="max-w-4xl space-y-10">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-zinc-500 mt-2 font-medium">Isi detail di bawah ini untuk memulai penggalangan dana baru.</p>
        </div>
    ...
    <form action="{{ route('admin.donasi.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-[3.5rem] shadow-sm border border-zinc-100 p-10 md:p-16 space-y-10" x-data="{ imageUrl: '' }">
        @csrf
        ...
        <div class="space-y-8">
                ...
                <div class="relative group">
                    <div class="h-[300px] border-2 border-dashed border-zinc-200 rounded-[3rem] flex flex-col items-center justify-center relative hover:bg-zinc-50 transition overflow-hidden group-hover:border-maroon-300">
                        ...
                        <div x-show="!imageUrl" class="flex flex-col items-center justify-center space-y-4">
                            ...
                            <p class="text-xs font-bold text-zinc-400">Klik atau seret foto ke sini</p>
                        </div>
            ...
            <div class="grid md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-5 px-8 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900" placeholder="Contoh: Bantuan untuk Korban Banjir">
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" rows="8" required class="w-full bg-zinc-50 border-none rounded-[2rem] py-6 px-8 focus:ring-2 focus:ring-maroon-500 font-medium text-zinc-600 leading-relaxed">{{ old('description') }}</textarea>
            ...
            <div class="flex items-center gap-4 p-8 bg-maroon-50 rounded-3xl border border-maroon-100">
                <div class="relative inline-block w-12 h-6 transition duration-200 ease-in-out bg-zinc-200 rounded-full">
                    <input type="checkbox" name="is_urgent" id="is_urgent" class="absolute w-6 h-6 bg-white border-2 border-zinc-200 rounded-full appearance-none cursor-pointer checked:right-0 checked:bg-maroon-600 checked:border-maroon-600 transition-all duration-200">
                </div>
                <div>
