...
<div class="space-y-10">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h1 class="text-3xl font-black text-zinc-900 tracking-tight">Riwayat <span class="text-maroon-700">Donasi Masuk</span></h1>
            <p class="text-zinc-500 mt-2 font-medium">Pantau dan konfirmasi transaksi donasi dari para orang baik.</p>
        </div>
    </div>
    ...
    <div class="flex justify-end">
        <form action="{{ route('admin.transactions.index') }}" method="GET" class="relative group w-full md:w-80">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-zinc-50">
                <thead class="bg-zinc-50/50">
                    <tr>
                        <th class="px-10 py-6 text-left text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">Donatur</th>
                ...
                <tbody class="divide-y divide-zinc-50">
                    ...
                    <tr class="hover:bg-zinc-50/50 transition">
                        <td class="px-10 py-6">
                            <div class="min-w-0">
                                <p class="text-sm font-black text-zinc-900 truncate max-w-[150px]">{{ $item->is_anonymous ? 'Hamba Allah' : ($item->donor_name ?? 'Anonim') }}</p>
                                <p class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{{ substr($item->transaction_id, 0, 12) }}</p>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                            <p class="text-xs font-bold text-zinc-900 truncate max-w-[200px]">{{ $item->campaign->title }}</p>
                        </td>
                        <td class="px-10 py-6">
                        ...
                        <td class="px-10 py-6">
                            @if($item->status == 'success')
                                <span class="px-3 py-1 bg-green-50 text-green-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-green-100">Berhasil</span>
                            @elseif($item->status == 'pending')
                                <span class="px-3 py-1 bg-amber-50 text-amber-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-amber-100">Menunggu</span>
                            @else
                                <span class="px-3 py-1 bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-red-100">Gagal</span>
                            @endif
                        </td>
                        <td class="px-10 py-6 text-right space-x-2">
                            @if($item->status == 'pending')
                            <form action="{{ route('admin.transactions.confirm', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Konfirmasi donasi ini sebagai BERHASIL?')">
                                @csrf
                            ...
                            @endif
                            <form action="{{ route('admin.transactions.destroy', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus data transaksi ini?')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-10 py-20 text-center text-zinc-400 font-medium">Belum ada transaksi donasi.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
