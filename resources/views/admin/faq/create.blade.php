...
<div class="max-w-4xl space-y-10">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-zinc-900 tracking-tight">Tambah <span class="text-maroon-700">FAQ</span></h1>
            <p class="text-zinc-500 mt-2 font-medium">Buat tanya jawab baru untuk membantu pengguna.</p>
        </div>
    ...
    <form action="{{ route('admin.faq.store') }}" method="POST" class="bg-white rounded-[3.5rem] shadow-sm border border-zinc-100 p-8 md:p-16 space-y-10">
        @csrf
        
        <div class="space-y-6">
            <div class="space-y-2">
                <label class="text-[10px] font-black text-zinc-400 uppercase tracking-widest pl-2">Pertanyaan</label>
                <input type="text" name="question" value="{{ old('question') }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900" placeholder="Contoh: Bagaimana cara berdonasi?">
            ...
            <div class="space-y-2">
                ...
                <textarea name="answer" rows="6" required class="w-full bg-zinc-50 border-none rounded-[2rem] py-6 px-8 focus:ring-2 focus:ring-maroon-500 font-medium text-zinc-600 leading-relaxed" placeholder="Berikan jawaban yang jelas dan mudah dipahami..."></textarea>
            ...
            <div class="grid md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <input type="number" name="order" value="{{ old('order', $nextOrder) }}" required class="w-full bg-zinc-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-maroon-500 font-bold text-zinc-900">
                </div>
                <div class="flex items-center gap-4 h-full pt-6">
                    <input type="checkbox" name="is_published" id="is_published" checked class="w-6 h-6 rounded-lg border-zinc-200 text-maroon-700 focus:ring-maroon-500">
