        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="space-y-4" x-data="{ active: null }">
                ...
                <div class="bg-white rounded-[2rem] border border-zinc-100 shadow-sm overflow-hidden transition-all duration-300" :class="active === {{ $item->id }} ? 'ring-2 ring-maroon-500 shadow-xl' : ''">
                    <button @click="active = (active === {{ $item->id }} ? null : {{ $item->id }})" class="w-full px-8 py-6 text-left flex items-center justify-between group">
                        <span class="text-lg font-bold text-zinc-900 group-hover:text-maroon-700 transition" :class="active === {{ $item->id }} ? 'text-maroon-700' : ''">{{ $item->question }}</span>
                        <div class="w-10 h-10 rounded-xl bg-zinc-50 flex items-center justify-center transition-colors" :class="active === {{ $item->id }} ? 'bg-maroon-50 text-maroon-600' : 'text-zinc-400'">
                            <svg class="w-6 h-6 transition-transform duration-300" :class="active === {{ $item->id }} ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 9l-7 7-7-7"></path></svg>
                        </div>
                    </button>
                    <div x-show="active === {{ $item->id }}" 
                         x-transition:enter="transition ease-out duration-300"
                         x-transition:enter-start="opacity-0 -translate-y-2"
                         x-transition:enter-end="opacity-100 translate-y-0"
                         class="px-8 pb-8 text-zinc-500 leading-relaxed font-medium whitespace-pre-line border-t border-zinc-50 pt-6">
                        {{ $item->answer }}
                    </div>
                </div>
                @empty
            ...
            <div class="mt-20 p-10 bg-maroon-900 rounded-[3rem] text-center relative overflow-hidden shadow-2xl">
                ...
                <div class="relative z-10">
                    ...
                    <a href="https://wa.me/{{ str_replace([' ', '-', '+'], '', \App\Models\Setting::first()->whatsapp ?? '') }}" class="inline-flex items-center gap-3 bg-amber-500 hover:bg-amber-400 text-maroon-950 px-10 py-4 rounded-2xl font-black transition transform active:scale-95 shadow-xl shadow-amber-900/40">
